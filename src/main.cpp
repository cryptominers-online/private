...
unsigned int nStakeMinAge = 21 * 24 * nOneHour; // 21 days as zero time weight
unsigned int nStakeMaxAge = 63 * 24 * nOneHour; // 63 days as full weight
unsigned int nStakeTargetSpacing = 3.5 * 60; // 3.5-minute stakes spacing
int64_t nTargetSpacing = 3.5 * 60;  // Same as the above
unsigned int nModifierInterval = 0.5 * nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "Bitcoin21 Signed Message:\n";
...
{
    int64_t nSubsidy = 21 * COIN;
	
	if (nHeight == 1)
		return 6300000 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 350000;
    if (halvings >= 64)
        return nFees;
...
{
    int64_t nRewardCoinYear = 0.0021 * CENT;
...
static const int64_t nTargetTimespan = 3.5 * 15 * 60;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("Bitcoin21-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Bitcoin21
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Bitcoin21: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bitcoin21", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Bitcoin21 Genesis Block";
        CTransaction txNew;
        txNew.nTime = 1459105015;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1459105017;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1347207; // !fTestNet ? 1575379 : 46534;
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd0e00fe772c405f156760beafc95f3282cb5493f0fd9fbd260be7aabaa169bfb"));

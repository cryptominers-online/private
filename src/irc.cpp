...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Bitcoin21-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #Bitcoin21TEST2\r");
            Send(hSocket, "WHO #Bitcoin21TEST2\r");
        } else {
            // randomly join #Bitcoin2100-#Bitcoin2105
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Bitcoin21%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bitcoin21%02d\r", channel_number).c_str());
        }
